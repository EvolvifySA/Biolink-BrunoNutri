...
class N {
  ...
  draw() {
    ...
    cx.fillStyle = this.act ? `rgba(122,226,58,${a})` : `rgba(60,120,60,${a * .7})`; cx.fill();
    if (this.act && p > .68) {
      cx.beginPath(); cx.arc(this.x, this.y, s * 3, 0, Math.PI * 2);
      cx.fillStyle = `rgba(122,226,58,${(p - .68) * .12})`; cx.fill();
    }
  }
}
...
function drawC() {
  for (let i = 0; i < nodes.length; i++) for (let j = i + 1; j < nodes.length; j++) {
    ...
    if (d < 130) {
      ...
      cx.strokeStyle = ac ? `rgba(122,226,58,${a})` : `rgba(60,120,60,${a * .5})`;
